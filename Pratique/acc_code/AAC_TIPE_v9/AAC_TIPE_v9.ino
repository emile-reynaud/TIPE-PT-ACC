...
const uint16_t KICK_DUREE_MS   = 150;    // durée de l'impulsion en ms
const float    VITESSE_QUASI_NULLE_MS = 0.1f;  // seuil "roue à l'arrêt" en m/s
...
void hallISR() {
  ...
  // ANTI-REBOND : Si l'impulsion arrive moins de 20ms (20000 µs) après la précédente,
  // c'est un rebond parasite mécanique ou électrique. On l'ignore.
...
float calculerVitessePeriode() {
  ...
  if (silenceActuel_us > HALL_TIMEOUT_US) {
    ...
    hallNouvelleImpulsion = false;
    interrupts();
    vitesse_periode_ms = 0.0f; // On force la vitesse filtrée à 0
    return 0.0f;
  }
  ...
  // Garde-fou sur valeur absurde
  if (v > 10.0f) return vitesse_periode_ms;

  // ── APPLICATION DU FILTRE PASSE-BAS EXPONENTIEL ──────────
  if (vitesse_periode_ms == 0.0f) {
    // Si la voiture était arrêtée, on démarre directement à la valeur brute
    vitesse_periode_ms = v;
  } else {
    // Filtre : 30% de la nouvelle mesure brute + 70% de l'ancienne vitesse filtrée.
    // Cela lisse l'incertitude des 23% sans introduire trop de retard de régulation.
...
float calculerCorrecteur(float erreur, float dt_s, bool reinitialiserDerivee = false) {
  ...
  float u_I = 0.0f;
#if USE_I
  // On ne fait PAS accumuler l'intégrale pendant le kick-start ni en urgence.
  // Pendant le kick, la vitesse est nulle et l'erreur est maximale :
  // si on laisse accumuler, l'intégrale atteint des valeurs énormes en
  // quelques cycles, et la commande reste saturée à 255 bien après le
  // démarrage → divergence assurée.
  // kickActif est déclaré extern implicitement via la portée globale.
  if (!kickActif) {
...
void loop() {
  ...
  if (now - lastControlTime >= CONTROL_PERIOD_MS) {
    ...
    // Variables statiques pour détecter les changements d'état
    static uint8_t ancienMode = 0; // 1 = Urgence, 2 = Vitesse, 3 = Distance
    ...
    if (dist_m > 0.0f && dist_m < DIST_URGENCE_M) {
      ...
      // ANTI-PIC DERIVE (Démarrage ou Changement de mode)
      if( modeActuel != ancienMode || (vitessePrecedente == 0.0f && vitesse_periode_ms > 0.0f)) {
    ...
    // kick-start si nécessaire + lissage par rampe.
    // On passe `now` pour que le kick puisse mesurer sa durée sans delay().
    commande = calculerCommandeMoteur(commande, dt_s, urgence, now);
    ...
    // kickActif remplace aideDemarragePWM depuis la v8
