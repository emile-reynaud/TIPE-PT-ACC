...
#define Kp_VAL   35.0f   // Augmenté : 12 PWM/m/s était insuffisant pour corriger
                          // les écarts résiduels du feedforward (erreur de modèle).
                          // À 0,1 m/s d'erreur : 35×0.1 = 3.5 PWM de correction.
#define Ki_VAL    5.0f   // Légèrement augmenté pour éliminer l'erreur statique plus vite
#define Kd_VAL    0.5f   // Inchangé — rester faible avec 3 aimants
...
float calculerCommandeMoteur(float commandeCorrecteur, float dt_s, bool urgence, bool interdireKick, uint32_t now) {
  ...
  const uint16_t KICK_GRACE_MS = 300; // 300 ms de grâce après un kick
  ...
  // Condition : la voiture est réellement à l'arrêt.
  // On exige que LES DEUX mesures de vitesse soient quasi nulles :
  //   - vitesse_periode_ms : précise mais ne se calcule qu'après 2 impulsions.
  //     Elle reste à 0 pendant ~87 ms après le démarrage (temps entre 2 aimants
  //     à 0,40 m/s : p/v = 0.087/0.40 ≈ 217 ms). Si on ne teste que cette
  //     valeur, le kick se redéclenche alors que la roue tourne déjà.
  //   - vitesse_ms (fréquencemétrique) : disponible immédiatement mais bruitée.
  //     Elle vaut 0 si aucune impulsion n'est comptée dans la fenêtre de 50 ms,
  //     ce qui arrive facilement à basse vitesse (< 1 aimant/50 ms).
  // Solution : on demande les deux à zéro ET on interdit le kick pendant
  // KICK_GRACE_MS après la fin du kick précédent — le temps que vitesse_periode
  // se mette à jour avec la première vraie mesure.
