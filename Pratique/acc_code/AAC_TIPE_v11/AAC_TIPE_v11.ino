 ...
 * Version 11 — Corrigée (Gestion des modes et sécurité Kick)
 * ============================================================
 *
 * ┌─ MATÉRIEL ──────────────────────────────────────────────┐
 * │  Arduino Nano Every                                      │
...
#define Kp_VAL   12.0f   // Gain proportionnel — réglé pour K=5.77, tau=116ms
#define Ki_VAL    3.0f   // Gain intégral — élimine l'erreur statique sans windup
#define Kd_VAL    0.5f   // Gain dérivé — faible car 3 aimants = mesure bruitée
...
const float VITESSE_CIBLE_MS   = 1.0f; // 1,0 m/s
...
float calculerCorrecteur(float erreur, float dt_s, bool reinitialiserDerivee) {
  // Terme proportionnel LINÉAIRE : u_P = Kp × e
  // ATTENTION : ne pas écrire erreur * abs(erreur) qui donne Kp×e²
  // (correcteur non-linéaire — comportement imprévisible et impossible à régler).
