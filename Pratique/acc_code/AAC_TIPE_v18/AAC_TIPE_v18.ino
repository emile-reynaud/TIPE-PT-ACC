...
float calculerVitessePeriode() {
  ...
  // - À la MONTÉE : filtre fort (alpha=0.25) → atténue les parasites Hall
  //   (rebonds mécaniques qui font apparaître une fausse vitesse élevée)
  // - À la DESCENTE : filtre faible (alpha=0.85) → suit rapidement la vraie
  //   décélération sans introduire de retard de phase.
  //
  // Pourquoi c'est important :
  // Avec un filtre symétrique à alpha=0.4, la descente de vitesse est retardée
  // de plusieurs cycles. Le correcteur voit une erreur négative persistante
  // (v_filtrée > v_réelle) et maintient une commande basse en zone morte.
  // L'intégrale accumule de l'erreur négative → undershoots → oscillations.
  // Un filtre asymétrique descend vite (pas de retard sur la correction) et
  // monte lentement (protection contre les parasites).
  float alpha;
  if (v_brute >= vitesse_periode_ms) {
    alpha = 0.25f;  // montée : filtre fort → anti-parasite
  } else {
    alpha = 0.85f;  // descente : filtre faible → pas de retard
  }

  if (vitesse_periode_ms == 0.0f) {
    vitesse_periode_ms = v_brute;
  } else {
    vitesse_periode_ms = alpha * v_brute + (1.0f - alpha) * vitesse_periode_ms;
  }
